...
// Folder and Pointer
std::string Texture::rootFolder = "Assets/Images/";
...
bool Texture::Load(const std::string& filename, const std::string& tag)
{
	...
	if (!imageData)
	{
		Utility::Log(Utility::Destination::WindowsMessageBox, "Error loading image file \"" + (rootFolder + filename) + "\"\n\nPossible causes could be a corrupt or missing file. Another reason could be that the filename and/or path are incorrectly spelt.", Utility::Severity::Failure);
		return false;
	}

	SDL_Texture* texture = SDL_CreateTextureFromSurface(Window::Instance()->GetRenderer(), imageData);
...
{	//Make sure you call 'SetTexture()' after loading the texture
	...
	if (!isAnimationDead)
	{
		...
		SDL_RenderCopyEx(Window::Instance()->GetRenderer(), texture, &src, &dst, angle, &centrePoint, static_cast<SDL_RendererFlip>(flip));
	}
}