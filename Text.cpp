...
bool Text::Initialize()
{
	if (TTF_Init() == -1)
	{
		Utility::Log(Utility::Destination::WindowsMessageBox, "Error initializing the font sub-system.", Utility::Severity::Failure);
		return false;
	}
...
bool Text::Load(const std::string& filename, const std::string& tag, FontSize fontSize)
{
	...
	if (!font)
	{
		Utility::Log(Utility::Destination::WindowsMessageBox, "Error loading font file \"" + (rootFolder + filename) + "\"\n\nPossible causes could be a corrupt or missing file. Another reason could be that the filename and/or path are incorrectly spelt.", Utility::Severity::Failure);
		return false;
	}
...
void Text::Render(int x, int y)
{
	...
	SDL_RenderCopy(Window::Instance()->GetRenderer(), texture, nullptr, &dst);
}
...
void Text::CreateText()
{
	...
	SDL_DestroyTexture(texture);
	texture = SDL_CreateTextureFromSurface(Window::Instance()->GetRenderer(), textSurface);
